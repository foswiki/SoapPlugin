...
sub prettyPrint {
    ...
    my ( $output, $exit ) =
      Foswiki::Sandbox->sysCommand( $xmllintCmd, INFILE => $xmlInFile->filename,
      );
...
sub formatResult {
    ...
    if ( $params->{raw} || $params->{verbatim} ) {
        my $content = $som->{_foswiki_content}
          || $som->context->transport->http_response->content;
...
sub fromUtf8 {
    ...
    if ( $] < 5.008 ) {
        ...
        if ( Unicode::MapUTF8::utf8_supported_charset($charset) ) {
            ...
            print STDERR
"Warning: Conversion from $charset no supported, ' . 'or name not recognised - check perldoc Unicode::MapUTF8\n";
            return $string;
        }
    }
    else {
        ...
        if ( not $encoding ) {
            print STDERR
"Warning: Conversion to $charset not supported, or name not recognised - check 'perldoc Encode::Supported'\n";
            return $string;
        }
        else {
...
"Warning: Conversion to $charset not supported, or name not recognised - check 'perldoc Encode::Supported'\n";
            ...
            my $octets =
              Encode::decode( $encoding, $string, &Encode::FB_PERLQQ() );
            $octets = Encode::encode( 'utf-8', $octets )
              unless utf8::is_utf8($octets);
            return $octets;
        }
    }
}
