...
sub run_for {
    ...
    eval
q!use XMLRPC::Lite on_fault => sub{ref $_[1] ? $_[1] : new XMLRPC::SOM}; 1!
      or die;

    print "Perl XMLRPC server test(s)...\n";

    $s = XMLRPC::Lite->proxy($proxy);

    ok( $s->call( 'My.Examples.getStateName', 1 )->result eq 'Alabama' );
    ok( $s->call( 'My.Examples.getStateNames', 1, 4, 6, 13 )->result =~
    ...
    $r =
      $s->call( 'My.Examples.getStateStruct', { item1 => 1, item2 => 4 } )
      ->result;
  ...
  ; 1" or die;
...

    {
        ...
        ok( $s->call('My.Parameters.die_simply')->faultstring =~
              /Something bad/ );
    ...
    {
        my @all = $s->call( 'My.Parameters.echo', 1 )->paramsall;
        ok( @all == 1 )
    }
    {
        my @all = $s->call( 'My.Parameters.echo', (1) x 10 )->paramsall;
        ok( @all == 10 )
    }
    ...
    if ( $proxy =~ /^tcp:/ ) {
        ...
        ok( $s->call( 'My.Examples.getStateName', 1 )->faultstring =~
