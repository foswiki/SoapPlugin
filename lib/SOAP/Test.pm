...
sub run_for {
    ...
    # ------------------------------------------------------
    my $s =
      SOAP::Lite->uri('http://something/somewhere')->proxy($proxy)
      ->on_fault( sub { } );
    ...
    eval q!use SOAP::Lite on_fault => sub{ref $_[1] ? $_[1] : new SOAP::SOM}; 1!
      or die;

    print STDERR "Perl SOAP server test(s)...\n";

    $s = SOAP::Lite->uri('urn:/My/Examples')->proxy($proxy);

    ok( $s->getStateName(1)->result eq 'Alabama' );
    ok( $s->getStateNames( 1, 4, 6, 13 )->result =~
    ...
    if ( $proxy =~ /^http/ ) {
        ...
        skip(
            'No Status checks for non http protocols on server side' => undef );
    }
  ...
  ; 1" or die;
    ...
    $s = SOAP::Lite->uri('urn:/My/PingPong')->proxy($proxy);
    ...
    {
        local $SOAP::Constants::NS_ENV =
          'http://schemas.xmlsoap.org/new/envelope/';
        my $s =
          SOAP::Lite->uri('http://my.own.site.com/My/Examples')->proxy($proxy)
          ->on_fault( sub { } );
        $r = $s->dosomething;
    ...
    if ( !ref $r || exists $r->{id} ) {
        ...
        skip(
            'No persistent objects (o-b-r) supported on server side' => undef );
    }
    ...
    {
        local
          $^W;    # disable warnings about deprecated AUTOLOADing for nonmethods
    ...
    if ( $proxy =~ /^tcp:/ ) {
        for ( 1 .. 2 ) {
            skip( 'No SOAPAction checks for tcp: protocol on server side' =>
                  undef );
        }
    }
    else {
    ...
    if ( !eval "pack('U*', 0)" ) {
        for (1) {
            skip( 'No UTF8 test. No support for pack("U*") modifier' => undef );
        }
    }
    else {
        $s =
          SOAP::Lite->uri('http://my.own.site.com/My/Parameters')
          ->proxy($proxy);
        ...
        ok(
            pack( 'U*', unpack( 'C*', $result ) ) eq
              $utf8 # should work where XML::Parser marks resulting strings as UTF-8
              || join( '', unpack( 'C*', $result ) ) eq
              join( '', unpack( 'C*', $utf8 ) )   # should work where it doesn't
        );
    }

    {
        ...
        my $s =
          SOAP::Lite->uri('http://my.own.site.com/My/Parameters')->proxy($proxy)
          ->on_fault( sub { $on_fault_was_called++; return } );
        ok( $s->die_simply()->faultstring =~ /Something bad/ );
        ok( $on_fault_was_called > 0 );
        my $detail =
          $s->die_with_object()->dataof( SOAP::SOM::faultdetail . '/[1]' );
    ...
    $s = SOAP::Lite->uri('')->proxy($proxy);

    ok( $s->getStateName(1)->faultstring =~
          /Denied access to method \(getStateName\) in class \(main\)/ );

    ok( $s->call( 'a:getStateName' => 1 )->faultstring =~
          /Denied access to method \(getStateName\) in class \(main\)/ );

    print STDERR "Number of parameters test(s)...\n";

    $s = SOAP::Lite->uri('http://my.own.site.com/My/Parameters')->proxy($proxy);
    ...
    ok(
        $s->call( SOAP::Data->name('a:getStateName')->uri('urn:/My/Examples'),
            1 )->result eq 'Alabama'
    );

    ok(
        $s->call(
            SOAP::Data->name('getStateName')->uri('urn:/My/Examples'), 1
          )->result eq 'Alabama'
    );

    ok(
        $s->call(
            SOAP::Data->name('a:getStateName')
              ->attr( { 'xmlns:a' => 'urn:/My/Examples' } ),
            1
          )->result eq 'Alabama'
    );
    ...
    SOAP::Trace->import(
        transport => sub {
            $_[0]->content_type('something/wrong')
              if UNIVERSAL::isa( $_[0] => 'HTTP::Request' );
        }
    );

    if ( $proxy =~ /^tcp:/ ) {
        skip( 'No Content-Type checks for tcp: protocol on server side' =>
              undef );
    }
    else {
